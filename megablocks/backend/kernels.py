...
@triton.jit
def batched_grouped_matmul_kernel(
    # device tensor of matrices pointers
    group_a_ptrs,
    group_b_ptrs,
    group_c_ptrs,
...
@triton.jit
def grouped_matmul_kernel_par_batch(
    # device tensor of matrices pointers
    group_a_ptrs,
    group_b_ptrs,
    group_c_ptrs,
...
@triton.jit
def grouped_matmul_kernel_debug(
    # device tensor of matrices pointers
    group_a_ptrs,
    group_b_ptrs,
    group_c_ptrs,
...
def grouped_matmul_kernel_opt(
    ...
    num_tiles = num_m_tiles * num_n_tiles
    k = gk
    ## Extract the iteration counts. ##
    num_iters = tl.cdiv(num_tiles-tile_idx, NUM_SM)
    k_tile_cnt = tl.cdiv(k, BLOCK_SIZE_K)
...
def group_gemm_fn(group_A, group_B, DEVICE):
    ...
    NUM_SM = 512
    grid = (NUM_SM,group_size)
    batched_grouped_matmul_kernel[grid](
        d_a_ptrs,
        d_b_ptrs,
        d_c_ptrs,
        d_g_sizes,
